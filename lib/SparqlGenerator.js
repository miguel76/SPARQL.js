...
Generator.prototype.input = function (input) {
  return 'INPUT ' +
      (input.name === '_' ? '' : '&' + input.name) +
        ((Object.keys(input.varMap).length > 1 || ! '_' in input.varMap) ?
        ' (' +
          Object.keys(input.varMap).map(inputVarname => {
            let localVarname = input.varMap[inputVarname].value;
            return (localVarname === inputVarname) ? '?' + inputVarname : ('(?' + inputVarname + ' AS ?' + localVarname + ')');
          }).join(' ') +
        ')':
        '' );
};
...
function variableToString(variable){
  return (variable.prefix || '?') + variable.value;
}
